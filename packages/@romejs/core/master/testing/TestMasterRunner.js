...
class BridgeDiagnosticsError extends diagnostics_1.DiagnosticsError {
    constructor(diag, bridge) {
        super(diag.description.message.value, [diag]);
        this.bridge = bridge;
    }
}
function grammarNumberTests(num) {
    return `<grammarNumber plural="tests" singular="test">${num}</grammarNumber>`;
}
function getProgressTestRefText(ref) {
    return string_markup_1.markup `<filelink target="${ref.filename}" />: ${string_markup_1.escapeMarkup(ref.testName)}`;
}
...
class TestMasterRunner {
    constructor(opts) {
        ...
        this.coverageCollector = new v8_1.CoverageCollector();
        this.progress = {
            totalTests: 0,
            startedTests: 0,
            finishedTests: 0,
        ...
        this.printer = opts.request.createDiagnosticsPrinter(this.request.createDiagnosticsProcessor({
            origins: [
                {
                    category: 'test',
    ...
    async processTestResult(ref, { inlineSnapshotUpdates, snapshotCounts }) {
        ...
        if (inlineSnapshotUpdates.length > 0) {
            ...
            inlineSnapshotUpdates = inlineSnapshotUpdates.map((update) => {
                const resolved = v8_1.sourceMapManager.resolveLocation(filename, update.line, update.column);
                if (resolved.filename !== filename && resolved.filename !== ref.uid) {
                    throw new Error(`Inline snapshot update resolved to ${resolved.filename} when it should be ${filename}`);
                }
                return {
                    ...update,
                    line: resolved.line,
            ...
            const diagnostics = await this.request.requestWorkerUpdateInlineSnapshots(path, inlineSnapshotUpdates, {});
    ...
    async spawnWorker(flags) {
        const proc = fork_1.default('test-worker', {
            stdio: 'pipe',
        ...
        const bridge = events_1.createBridgeFromChildProcess(core_1.TestWorkerBridge, proc, {
            type: 'client',
        });
        ...
        if (inspectorUrl !== undefined) {
            const client = new v8_1.InspectorClient(await codec_websocket_1.createClient(inspectorUrl));
            inspector = client;
    ...
    async setupWorkers() {
        ...
        for (let i = 0; i < constants_1.MAX_WORKER_COUNT; i++) {
    ...
    async init() {
        ...
        const callbacks = await Promise.all(workerContainers.map((container) => this.prepareWorker(container, progress)));
        progress.end();
        // Run tests
    ...
    async handleWorkerTimeout(duration, container) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                resolve(container.bridge.end(`Test worker was unresponsive for ${duration}. We tried to collect some additional metadata but we timed out again trying to fetch it...`));
            }, 3000);
            this._handleWorkerTimeout(duration, container).then(() => {
                ...
                clearTimeout(timeout);
                if (err instanceof v8_1.InspectorClientCloseError) {
                    return container.bridge.end(`Test worker was unresponsive for ${duration}. We tried to collect some additional metadata but the inspector connection closed abruptly`);
                }
                else {
                    reject(err);
                }
    ...
    async _handleWorkerTimeout(duration, { bridge, inspector }) {
        ...
        for (const callFrame of callFrames) {
            ...
            const resolved = v8_1.sourceMapManager.resolveLocation(v8_1.urlToFilename(callFrame.get('url').asString()), ob1_1.ob1Coerce0To1(loc.get('lineNumber').asZeroIndexedNumber()), loc.get('columnNumber').asZeroIndexedNumber());
        ...
        bridge.endWithError(new BridgeDiagnosticsError(diagnostics_1.deriveDiagnosticFromErrorStructure({
            name: 'Error',
            frames,
        }, {
            description: {
                category: 'tests/timeout',
                message: diagnostics_1.createBlessedDiagnosticMessage(`Test worker was unresponsive for <emphasis>${duration}</emphasis>. Possible infinite loop. Below is a stack trace before the test was terminated.`),
                advice: [
                    {
                        type: 'log',
                        category: 'info',
            ...
            bridge.endEvent.subscribe((error) => {
                ...
                for (const ref of cancelTests) {
                    ...
                    if (cancelTests.length === 1) {
                        ...
                        const errDiag = diagnostics_1.deriveDiagnosticFromError(error, {
                            label: ref.testName,
                        ...
                        this.printer.processor.addDiagnostic({
                            label: ref.testName,
                            description: diagnostics_1.descriptions.TESTS.CANCELLED,
                            location: {
                                filename: ref.filename,
                            },
                        });
    ...
    printCoverageReport(isError) {
        ...
        function buildRows(folder, depth) {
            ...
            const folderPercent = utils_1.percentInsideCoverageFolder(folder);
            rows.push([
                ' '.repeat(depth) + `<emphasis>${name}</emphasis>`,
                utils_1.formatPercent(folderPercent.functions),
                utils_1.formatPercent(folderPercent.branches),
                utils_1.formatPercent(folderPercent.lines),
            ]);
            ...
            for (const [name, file] of utils_1.sortMapKeys(folder.files)) {
                ...
                rows.push([
                    fileIndent + string_markup_1.markup `<filelink target="${absolute}">${name}</filelink>`,
                    utils_1.formatPercent(file.functions.percent),
                    utils_1.formatPercent(file.branches.percent),
                    utils_1.formatPercent(file.lines.percent),
                ]);
            }
            for (const subFolder of utils_1.sortMapKeys(folder.folders).values()) {
        ...
        if (!showAllCoverage) {
            reporter.br();
            reporter.info('Additional coverage information available. Refine the executed tests or add the <emphasis>--show-all-coverage</emphasis> flag');
        }
        reporter.hr();
    }
    ...
    printFocusedTestWarning(reporter) {
        ...
        const formattedFocusedTests = focusedTests.map(({ testName, location }) => {
            ...
            return string_markup_1.markup `<emphasis>${testName}</emphasis> at <emphasis>${string_markup_1.safeMarkup(diagnostics_1.diagnosticLocationToMarkupFilelink(loc))}</emphasis>`;
        });
        if (focusedTests.length === 1) {
            reporter.warn(`Only ran the focused test ${formattedFocusedTests[0]}`);
        }
        else {
            reporter.warn(`Only ran the following <number emphasis>${focusedTests.length}</number> focused ${grammarNumberTests(focusedTests.length)}`);
            reporter.list(formattedFocusedTests);
        }
        const otherTotal = this.progress.totalTests - this.focusedTests.length;
        reporter.info(`<number emphasis>${otherTotal}</number> other ${grammarNumberTests(otherTotal)} ignored`);
    }
    printSnapshotCounts(reporter) {
        const { createdSnapshots, deletedSnapshots, updatedSnapshots, updatedInlineSnapshots, } = this.progress;
    ...
    throwPrinter() {
        ...
        printer.onFooterPrint((reporter, isError) => {
            ...
            if (!isError) {
                ...
                reporter.success(`${prefix}<emphasis>${string_utils_1.humanizeNumber(totalCount)}</emphasis> ${grammarNumberTests(totalCount)} passed!`);
                return true;
            }
            // Show default footer
            return false;
...
exports.default = TestMasterRunner;
