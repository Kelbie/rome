...
const Resolver_1 = require("./fs/Resolver");
const FileAllocator_1 = require("./fs/FileAllocator");
const Logger_1 = require("../common/utils/Logger");
const MemoryFileSystem_1 = require("./fs/MemoryFileSystem");
...
const MasterReporter_1 = require("./MasterReporter");
const VirtualModules_1 = require("./fs/VirtualModules");
...
async function validateRequestFlags(req, masterCommand) {
        ...
        for (const key of disallowedFlagsWhenReviewing) {
            if (requestFlags[key]) {
                throw req.throwDiagnosticFlagError({
                    description: diagnostics_1.descriptions.FLAGS.DISALLOWED_REVIEW_FLAG(key),
...
function validateAllowedRequestFlag(req, flagKey, masterCommand) {
    ...
    if (req.query.requestFlags[flagKey] && !allowRequestFlags.includes(flagKey)) {
        throw req.throwDiagnosticFlagError({
            description: diagnostics_1.descriptions.FLAGS.DISALLOWED_REQUEST_FLAG(flagKey),
...
class Master {
    constructor(opts) {
        ...
        this.fileChangeEvent = new events_1.Event({
            name: 'Master.fileChange',
            onError: this.onFatalErrorBound,
        });
        this.clientStartEvent = new events_1.Event({
            name: 'Master.clientStart',
            onError: this.onFatalErrorBound,
        });
        this.requestStartEvent = new events_1.Event({
            name: 'Master.requestStart',
            onError: this.onFatalErrorBound,
        });
        this.logEvent = new events_1.Event({
            name: 'Master.log',
            onError: this.onFatalErrorBound,
        });
        ...
        this.logger = new Logger_1.default('master', () => {
            ...
            markupOptions: {
                humanizeFilename: (filename) => {
                    const path = path_1.createUnknownFilePath(filename);
                    if (path.isAbsolute()) {
                        const remote = this.projectManager.getRemoteFromLocalPath(path.assertAbsolute());
                        if (remote !== undefined) {
                            return remote.join();
                        }
                    }
        ...
        this.connectedReporters = new MasterReporter_1.default(this);
        this.connectedClientsListeningForLogs = new Set();
        this.connectedLSPServers = new Set();
        this.connectedClients = new Set();
        this.virtualModules = new VirtualModules_1.default(this);
        this.memoryFs = new MemoryFileSystem_1.default(this);
        this.projectManager = new ProjectManager_1.default(this);
        this.workerManager = new WorkerManager_1.default(this);
        this.fileAllocator = new FileAllocator_1.default(this);
    ...
    onFatalError(err) {
        const message = `<emphasis>Fatal error occurred</emphasis>: ${string_markup_1.escapeMarkup(err.stack || err.message)}`;
    ...
    async handleDisconnectedDiagnostics(diagnostics) {
        this.connectedReporters.error('Generated diagnostics without a current request');
    ...
    readDiagnosticsPrinterFile(path) {
        ...
        if (remoteToLocal === undefined) {
            return cli_diagnostics_1.readDiagnosticsFileLocal(path);
        }
        else {
            return cli_diagnostics_1.readDiagnosticsFileLocal(remoteToLocal);
        }
    }
    ...
    maybeSetupGlobalErrorHandlers() {
        ...
        const teardown = setupGlobalErrorHandlers_1.default((err) => {
    ...
    async attachToBridge(bridge) {
        ...
        if (client.version !== constants_1.VERSION) {
            // TODO this wont ever actually be printed?
            client.reporter.error(`Client version ${client.version} does not match server version ${constants_1.VERSION}. Goodbye lol.`);
            client.bridge.end();
            return;
        }
    ...
    async createClient(bridge) {
        const { flags: rawFlags, useRemoteReporter, hasClearScreen, columns, unicode, format, version, } = await bridge.getClientInfo.call();
        // Turn the cwd back into a AbsoluteFilePath
        const flags = {
            ...rawFlags,
            cwd: path_1.createAbsoluteFilePath(rawFlags.cwd),
        };
        const outStream = {
            type: 'out',
            columns,
    ...
    async handleRequest(client, partialQuery) {
        ...
        const req = new MasterRequest_1.default({
            client,
            query,
            master: this,
        });
        ...
        try {
            ...
            if (res === undefined) {
                throw new Error('teardown should have returned a normalized MasterQueryResponse');
            }
            return res;
        }
        catch (err) {
    ...
    async dispatchBenchmarkRequest(req, bridgeEndPromise) {
        ...
        reporter.section('Benchmark results', () => {
            reporter.info('Request artifacts may have been cached after the first run, artificially decreasing subsequent run time');
    ...
    async dispatchRequest(req, bridgeEndPromise, origins) {
        ...
        try {
            ...
            const flagsConsumer = consume_1.consume({
                ...
                context: {
                    ...
                    normalizeKey: (key) => {
                        return string_utils_1.toKebabCase(key);
                    },
            ...
            // Get command
            const masterCommand = commands_1.masterCommands.get(query.commandName);
            if (masterCommand) {
                // Warn about disabled disk caching
                if (process.env.ROME_CACHE === '0' &&
                    !this.warnedCacheClients.has(bridge)) {
                    reporter.warn('Disk caching has been disabled due to the <emphasis>ROME_CACHE=0</emphasis> environment variable');
                ...
                return {
                    ...MasterRequest_1.EMPTY_SUCCESS_RESPONSE,
            ...
            else if (err instanceof MasterRequest_1.MasterRequestCancelled) {
                return {
                    type: 'CANCELLED',
                };
            }
            else if (err instanceof MasterRequest_1.MasterRequestInvalid) {
                return {
    ...
    handleRequestError(req, rawErr) {
        ...
        const diagnostics = diagnostics_1.getDiagnosticsFromError(err);
        if (diagnostics !== undefined) {
        ...
        if (err instanceof cli_diagnostics_1.DiagnosticsPrinter) {
            ...
            if (rawErr instanceof MasterRequest_1.MasterRequestInvalid) {
                shouldPrint = true;
            }
            if (!req.bridge.alive) {
                shouldPrint = false;
        ...
        const errorDiag = diagnostics_1.deriveDiagnosticFromError(err, {
            description: {
        ...
        printer.processor.addDiagnostic({
            ...
            description: {
                ...
                advice: [...errorDiag.description.advice, diagnostics_1.INTERNAL_ERROR_LOG_ADVICE],
            },
        });
        printer.print();
