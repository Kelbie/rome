...
const vcs_1 = require("@romejs/vcs");
function cleanUidParts(parts) {
    let uid = '';
    let lastPart = '';
...
function cleanRelativeUidPath(relative) {
...
class ProjectManager {
    constructor(master) {
        ...
        this.projectFolderToId = new path_1.AbsoluteFilePathMap();
        this.projectConfigDependenciesToIds = new path_1.AbsoluteFilePathMap();
        this.fileToProject = new path_1.AbsoluteFilePathMap();
        this.projects = new Map();
        // We maintain these maps so we can reverse any uids, and protect against collisions
        this.uidToFilename = new Map();
        this.filenameToUid = new path_1.AbsoluteFilePathMap();
        this.remoteToLocalPath = new path_1.UnknownFilePathMap();
        this.localPathToRemote = new path_1.AbsoluteFilePathMap();
    }
    async init() {
        this.master.memoryFs.deletedFileEvent.subscribe((path) => {
            this.handleDeleted(path);
        });
        const vendorProjectConfig = {
            ...project_1.DEFAULT_PROJECT_CONFIG,
            name: 'rome-internal-remote',
        };
        const defaultVendorPath = vendorProjectConfig.files.vendorPath;
        await fs_1.createDirectory(defaultVendorPath, { recursive: true });
    ...
    getFilePathFromUidOrAbsolute(uid) {
        ...
        const path = path_1.createAbsoluteFilePath(uid);
        if (path.isAbsolute()) {
    ...
    normalizeFilenamesToFilePaths(filenames) {
        ...
        const absolutes = new path_1.AbsoluteFilePathSet();
    ...
    getUid(path) {
        ...
        while (true) {
            ...
            if (pkg === undefined || pkg.folder.equal(project.folder)) {
                ...
                const name = codec_js_manifest_1.manifestNameToString(pkg.manifest.name);
                if (name !== undefined) {
                    parts.unshift(name);
    ...
    async evictProject(project) {
        ...
        await Promise.all(ownedFiles.map((path) => this.master.fileAllocator.evict(path)));
    ...
    findProjectConfigConsumer(def, test) {
        const meta = project_1.assertHardMeta(def.meta);
    ...
    async getVCSClient(project) {
        ...
        if (client === undefined) {
            const { value: rootConfigConsumer, consumer, } = this.findProjectConfigConsumer(project, (consumer) => consumer.has('vsc') && consumer.get('vsc').get('root'));
            ...
            throw diagnostics_1.createSingleDiagnosticError({
                description: diagnostics_1.descriptions.PROJECT_MANAGER.NO_VCS(rootConfigLocation),
                location,
            });
        }
        else {
    ...
    async maybeGetVCSClient(project) {
        return await vcs_1.getVCSClient(project.config.vcs.root);
    }
    async addProject(projectFolder, configPath) {
        const { config, meta } = project_1.loadCompleteProjectConfig(projectFolder, configPath);
    ...
    async addProjectWithConfig({ projectFolder, meta, config, }) {
    ...
    declareManifest(project, isProjectPackage, def, diagnostics) {
        const name = codec_js_manifest_1.manifestNameToString(def.manifest.name);
        ...
        if (isProjectPackage && name !== undefined) {
            for (const project of projects) {
                ...
                diagnostics.addDiagnostic({
                    description: diagnostics_1.descriptions.PROJECT_MANAGER.DUPLICATE_PACKAGE(name, existingPackage.path.join()),
    ...
    async notifyWorkersOfProjects(workers, projects) {
        ...
        for (const project of projects) {
            projectsSerial.push({
                config: project_1.serializeJSONProjectConfig(project.config),
                id: project.id,
                folder: project.folder.join(),
            });
    ...
    async assertProject(path, location) {
        ...
        if (location === undefined) {
            throw new Error(`Couldn't find a project. Checked ${project_1.ROME_CONFIG_FILENAMES.join(' or ')} for ${path.join()}`);
        }
        throw diagnostics_1.createSingleDiagnosticError({
            location,
            description: diagnostics_1.descriptions.PROJECT_MANAGER.NOT_FOUND,
        });
    }
    ...
    getHandlerWithProject(path) {
        ...
        if (project === undefined) {
            ...
            return fileHandlers_1.getFileHandler(path, project.config);
        }
    }
    ...
    async findProject(cwd) {
        ...
        for (const dir of parentDirectories) {
            ...
            for (const configFilename of project_1.ROME_CONFIG_FILENAMES) {
                ...
                const configPathNested = dir.append(project_1.ROME_CONFIG_FOLDER).append(configFilename);
                const hasProjectNested = await this.master.memoryFs.existsHard(configPathNested);
                if (hasProjectNested) {
            ...
            if (await this.master.memoryFs.existsHard(packagePath)) {
                const input = await fs_1.readFileText(packagePath);
                const json = await codec_json_1.consumeJSON({ input, path: packagePath });
                if (json.has(project_1.ROME_CONFIG_PACKAGE_JSON_FIELD)) {
        ...
        for (const dir of parentDirectories) {
            for (const basename of project_1.ROME_CONFIG_WARN_FILENAMES) {
                const path = dir.append(basename);
                if (await this.master.memoryFs.existsHard(path)) {
                    this.warnIncorrectConfigFile(path, diagnostics_1.DiagnosticsProcessor.createImmediateThrower([
                        {
    ...
    checkConfigFile(path, diagnostics) {
        if (project_1.ROME_CONFIG_WARN_FILENAMES.includes(path.getBasename())) {
            this.warnIncorrectConfigFile(path, diagnostics);
        }
    }
    warnIncorrectConfigFile(path, diagnostics) {
        diagnostics.addDiagnostic({
            description: diagnostics_1.descriptions.PROJECT_MANAGER.INCORRECT_CONFIG_FILENAME(project_1.ROME_CONFIG_FILENAMES),
            location: {
                filename: path.join(),
            },
        });
