...
function isWordStartChar(char) {
    return parser_core_1.isAlpha(char) || char === '_' || char === '$';
}
// But a digit can appear inside of a word
function isWordChar(char) {
    return isWordStartChar(char) || parser_core_1.isDigit(char);
}
...
function isntBlockCommentEnd(char, index, input) {
...
function isNumberChar(char) {
    return parser_core_1.isDigit(char) || char === '_';
}
exports.createJSONParser = parser_core_1.createParser((ParserCore) => class JSONParser extends ParserCore {
    constructor(opts) {
        super({
            ...opts,
    ...
    tokenize(index, input) {
        ...
        // Line comment
        if (char === '/' && nextChar === '/') {
            const commentValueIndex = ob1_1.ob1Add(index, 2);
            const [value] = this.readInputFrom(commentValueIndex, isntNewline);
            // (comment content start + comment content length)
            return this.finishValueToken('LineComment', value, ob1_1.ob1Add(commentValueIndex, value.length));
        }
        // BlockComment
        if (char === '/' && nextChar === '*') {
            const commentValueIndex = ob1_1.ob1Add(index, 2);
            ...
            const endIndex = ob1_1.ob1Add(ob1_1.ob1Add(commentValueIndex, value.length), 2);
            // Ensure the comment is closed
            if (this.input[ob1_1.ob1Get0(endIndex) - 2] !== '*' ||
                this.input[ob1_1.ob1Get0(endIndex) - 1] !== '/') {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.UNCLOSED_BLOCK_COMMENT,
        ...
        switch (char) {
            case '"': {
                const [value] = this.readInputFrom(ob1_1.ob1Inc(index), isStringValueChar);
                // Check for closed string (index is the current token index + string length + closing quote + 1 for the end char)
                const end = ob1_1.ob1Add(ob1_1.ob1Add(index, value.length), 2);
                if (input[ob1_1.ob1Get0(end) - 1] !== '"') {
                    throw this.unexpected({
                        description: diagnostics_1.descriptions.JSON.UNCLOSED_STRING,
                ...
                for (let strIndex = 0; strIndex < value.length; strIndex++) {
                    ...
                    if (char === '\n') {
                        throw this.unexpected({
                            description: diagnostics_1.descriptions.JSON.STRING_NEWLINES_IN_JSON,
                            start: this.getPositionFromIndex(ob1_1.ob1Add(index, strIndex)),
                        });
                    }
                }
                // Unescape the string
                const unescaped = string_escape_1.unescapeString(value, (metadata, strIndex) => {
                    throw this.unexpected({
            ...
            case "'":
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.SINGLE_QUOTE_USAGE,
                    start: this.getPositionFromIndex(index),
                });
            case '/':
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.REGEX_IN_JSON,
                    start: this.getPositionFromIndex(index),
                });
            case ',':
        ...
        // Numbers
        if (parser_core_1.isDigit(char)) {
            const value = this.removeUnderscores(index, this.readInputFrom(index, isNumberChar)[0]);
            const num = Number(value);
            return this.finishValueToken('Number', num, ob1_1.ob1Add(index, value.length));
        }
        // Word - boolean, undefined etc
        if (isWordStartChar(char)) {
            const [value] = this.readInputFrom(index, isWordChar);
            return this.finishValueToken('Word', value, ob1_1.ob1Add(index, value.length));
        }
        // Unknown character
        return undefined;
    }
    parseObject(firstKeyStart, firstKey) {
        ...
        do {
            ...
            if (this.matchToken('Comma')) {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.REDUNDANT_COMMA,
                });
            }
        ...
        for (let i = 0; i < raw.length; i++) {
            ...
            if (char === '_') {
                ...
                if (!this.hasExtensions) {
                    throw this.unexpected({
                        description: diagnostics_1.descriptions.JSON.NUMERIC_SEPARATORS_IN_JSON,
                        start: this.getPositionFromIndex(ob1_1.ob1Inc(index)),
                    });
                }
            }
            else {
    ...
    eatComments() {
        ...
        while (true) {
            ...
            if (!this.hasExtensions) {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.COMMENTS_IN_JSON,
                });
            }
            this.nextToken();
        }
    ...
    parseArray() {
        ...
        do {
            ...
            if (this.matchToken('Colon')) {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.MISTAKEN_ARRAY_IDENTITY,
                });
            }
        } while (this.eatPropertySeparator());
    ...
    eatPropertySeparator() {
        ...
        if (this.hasExtensions) {
            ...
            if (lookahead.type === 'BraceClose' || lookahead.type === 'BracketClose') {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.TRAILING_COMMA_IN_JSON,
                });
            }
            this.nextToken();
            return true;
    ...
    parseWord(isStart) {
        ...
        switch (token.value) {
            ...
            case 'undefined':
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.UNDEFINED_IN_JSON,
                });
        }
        if (isStart && this.matchToken('Colon')) {
            if (this.hasExtensions) {
                ...
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.IMPLICIT_OBJECT_IN_JSON,
                });
            }
        }
        throw this.unexpected({
            description: diagnostics_1.descriptions.JSON.UNKNOWN_WORD_IN_JSON(token.value),
        });
    }
    parseNumber() {
        ...
        if (nextToken2.type === 'Word' && nextToken2.value === 'n') {
            throw this.unexpected({
                description: diagnostics_1.descriptions.JSON.BIGINT_IN_JSON,
            });
        }
        ...
        switch (token.type) {
            ...
            case 'Word':
                if (this.hasExtensions) {
                    ...
                    throw this.unexpected({
                        description: diagnostics_1.descriptions.JSON.PROPERTY_KEY_UNQUOTED_IN_JSON,
                    });
                }
            default:
                throw this.unexpected();
    ...
    parseEntry() {
        if (this.matchToken('EOF')) {
            if (this.hasExtensions) {
                ...
                throw this.unexpected({
                    description: diagnostics_1.descriptions.JSON.EMPTY_INPUT_IN_JSON,
                });
            }
        }
        else {
                ...
                if (target === 'inner-value') {
                    ...
                    if (typeof originalValue === 'string') {
                        loc = {
                            ...
                            start: {
                                ...loc.start,
                                column: ob1_1.ob1Add(loc.start.column, 1),
                            },
                            end: {
                                ...loc.end,
                                column: ob1_1.ob1Sub(loc.end.column, 1),
                            },
                        };
                    }
                }
