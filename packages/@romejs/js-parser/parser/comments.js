...
function setComments(node, key, comments) {
    let innerEndIndex = -1;
...
function addComment(parser, comment) {
    parser.state.trailingComments.push(comment);
    parser.state.leadingComments.push(comment);
}
exports.addComment = addComment;
function adjustCommentsAfterTrailingComma(parser, node, elements, 
...
function attachComments(parser, node) {
    ...
    if (parser.state.trailingComments.length > 0) {
        ...
        if (hasComments(lastInStack.trailingComments) &&
            start(parser.comments.assertGetCommentFromId(lastInStack.trailingComments[0])) >= end(node)) {
            trailingComments = parser.comments.getCommentsFromIds(lastInStack.trailingComments);
    ...
    if (firstChild) {
        ...
        ((commentPreviousNode.type === 'ImportSpecifier' &&
            ...
            (commentPreviousNode.type === 'ExportExternalSpecifier' &&
                node.type !== 'ExportExternalSpecifier'))) {
        adjustCommentsAfterTrailingComma(parser, node, [parser.state.commentPreviousNode]);
    }
    if (lastChild !== undefined) {
        if (hasComments(lastChild.leadingComments)) {
            if (lastChild !== node &&
                end(parser.comments.assertGetCommentFromId(last(lastChild.leadingComments))) <= start(node)) {
                setComments(node, 'leadingComments', parser.comments.getCommentsFromIds(lastChild.leadingComments));
                ...
                for (let i = lastChild.leadingComments.length - 2; i >= 0; --i) {
                    if (end(parser.comments.assertGetCommentFromId(lastChild.leadingComments[i])) <=
                        start(node)) {
                        setComments(node, 'leadingComments', parser.comments.getCommentsFromIds(lastChild.leadingComments.splice(0, i + 1)));
                        break;
                    }
                }
            }
        ...
        if (end(last(parser.state.leadingComments)) <= start(node)) {
            if (parser.state.commentPreviousNode) {
                for (let j = 0; j < parser.state.leadingComments.length; j++) {
                    if (end(parser.state.leadingComments[j]) <
