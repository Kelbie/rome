...
rome_1.test('parse', async (t) => {
    ...
    for (const str of versionTests) {
        t.snapshot(codec_semver_1.parseSemverVersion({ input: str }));
    }
    // loose mode
    const looseRangeTests = ['* - 1.2.3', 'v1.2.3', '||', '', '1.2.3prerelease'];
    for (const str of looseRangeTests) {
        t.snapshot(codec_semver_1.parseSemverRange({ input: str, loose: true }));
        t.throws(() => {
            codec_semver_1.parseSemverRange({ input: str, loose: false });
        });
    }
    ...
    for (const str of rangeTests) {
        t.snapshot(codec_semver_1.parseSemverRange({ input: str }));
    }
    // ensure ranges throw in version mode
    for (const str of rangeTests) {
        t.throws(() => {
            codec_semver_1.parseSemverVersion({ input: str });
        });
    }
});
