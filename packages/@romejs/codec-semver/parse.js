...
const createSemverParser = parser_core_1.createParser((ParserCore) => class SemverParser extends ParserCore {
    ...
    tokenize(index, input) {
        ...
        if ((char === '<' && nextChar === '=') ||
            ...
            return this.finishValueToken('Operator', value, ob1_1.ob1Add(index, 2));
        }
        if (char === '^' ||
            char === '<' ||
            char === '>' ||
        ...
        if (char === '|' && nextChar === '|') {
            return this.finishToken('Pipe', ob1_1.ob1Add(index, 2));
        }
        if (char === '*') {
            return this.finishToken('Star');
        }
        if (input[ob1_1.ob1Get0(index) - 1] === ' ' && char === '-' && nextChar === ' ') {
        ...
        if (parser_core_1.isDigit(char)) {
            const [value] = this.readInputFrom(index, parser_core_1.isDigit);
            return this.finishValueToken('Number', Number(value), ob1_1.ob1Add(index, value.length));
        }
        if (parser_core_1.isAlpha(char)) {
            const [value] = this.readInputFrom(index, parser_core_1.isAlpha);
    ...
    parseVersion() {
        ...
        if (this.eatToken('Dot')) {
            ...
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.MISSING_MINOR_VERSION,
            });
        }
        if (this.eatToken('Dot')) {
            ...
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.MISSING_PATCH_VERSION,
            });
        }
        if (this.matchToken('Dot')) {
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.EXCESSIVE_VERSION_PARTS,
            });
        }
    ...
    parseVersionQualifierPart() {
        ...
        do {
            ...
            if (token.type === 'Number' || token.type === 'Word') {
                ...
                throw this.unexpected({
                    description: diagnostics_1.descriptions.SEMVER.INVALID_QUANTIFIER_PART,
                });
            }
    ...
    parseVersionNumber() {
        ...
        if (this.isWildcardToken(token)) {
            if (this.mode === 'version') {
                throw this.unexpected({
                    description: diagnostics_1.descriptions.SEMVER.WILDCARD_IN_VERSION,
                });
            }
            this.nextToken();
        }
            ...
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.INVALID_VERSION_NUMBER,
            });
        }
        return undefined;
    }
    ...
    validateRangeSide(node) {
        ...
        throw this.unexpected({
            ...diagnostics_1.descriptions.SEMVER.INVALID_RANGE,
    ...
    parseAtomStartPipe() {
        if (this.loose) {
            ...
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.BARE_PIPE_WITHOUT_LOOSE,
            });
        }
    }
    parseAtomStartWord(token) {
        if (this.isWildcardToken(token)) {
            ...
            throw this.unexpected({
                description: diagnostics_1.descriptions.SEMVER.UNEXPECTED_WORD(token.value),
            });
        }
    }
    parseAtom() {
        ...
        switch (token.type) {
            ...
            default:
                throw this.unexpected({
                    description: diagnostics_1.descriptions.SEMVER.UNKNOWN_START,
                });
        }
    }
    ...
    parseInitialVersion() {
        ...
        if (node.type !== 'AbsoluteVersion') {
            throw this.unexpected({
                ...diagnostics_1.descriptions.SEMVER.EXPECTED_VERSION,
                start: this.getLoc(node).start,
            });
        }
        return node;
