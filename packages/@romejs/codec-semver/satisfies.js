...
function compareOp(op, version, range) {
    if (range.type === 'Wildcard') {
        return true;
    }
    switch (op) {
        case '=':
            return compare_1.compareFromAst(version, range) === 0;
        case '<':
            return compare_1.compareFromAst(version, range) < 0;
        case '>':
            return compare_1.compareFromAst(version, range) > 0;
...
function satisfiesFromAst(version, range) {
...
exports.satisfiesFromAst = satisfiesFromAst;
function satisfiesSub(version, range) {
    switch (range.type) {
        case 'AbsoluteVersion':
        case 'WildcardVersion':
        ...
        case 'LogicalAnd':
            return (satisfiesSub(version, range.left) && satisfiesSub(version, range.right));
        case 'LogicalOr':
            return (satisfiesSub(version, range.left) || satisfiesSub(version, range.right));
        case 'VersionRange':
            return (inRange(version, range.left, range.right) ||
