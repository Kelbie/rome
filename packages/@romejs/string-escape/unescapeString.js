...
function unescapeChar(modifier) {
    switch (modifier) {
        case 'b':
            return '\b';
        case 'f':
...
const UNEXPECTED_DEFAULT_THROWER = (metadata, index) => {
    throw new TypeError(`${metadata.message.value} (${String(index)})`);
};
function unescapeString(input, unexpected = UNEXPECTED_DEFAULT_THROWER) {
    let buffer = '';
    let index = 0;
    while (index < input.length) {
        ...
        if ((isEscaped && char === '\\') || char !== '\\') {
            ...
            if (codePoint >= 0 && codePoint <= 31) {
                throw unexpected(diagnostics_1.descriptions.STRING_ESCAPE.INVALID_STRING_CHARACTER, index);
            }
            // Add it verbatim
            buffer += char;
            index++;
        ...
        if (modifier === 'u') {
            ...
            if (rawCode.length < 4) {
                ...
                throw unexpected(diagnostics_1.descriptions.STRING_ESCAPE.NOT_ENOUGH_CODE_POINTS, lastDigitIndex);
            }
            // Validate that each character is a valid hex digit
            for (let i = 0; i < rawCode.length; i++) {
                const char = rawCode[i];
                if (!parser_core_1.isHexDigit(char)) {
                    // Get the current source index for this character
                    // (code start index + digit index)
                    const pos = codeStartIndex + i;
                    throw unexpected(diagnostics_1.descriptions.STRING_ESCAPE.INVALID_HEX_DIGIT_FOR_ESCAPE, pos);
                }
            }
