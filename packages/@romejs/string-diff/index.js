...
const DIFF_INSERT = 1;
const DIFF_EQUAL = 0;
exports.diffConstants = {
    DELETE: DIFF_DELETE,
    EQUAL: DIFF_EQUAL,
    ADD: DIFF_INSERT,
};
function groupDiffByLines(rawDiffs) {
    const diffsByLine = [];
...
exports.groupDiffByLines = groupDiffByLines;
function stringDiff(text1, text2) {
 ...
 */
function main(text1, text2, fixUnicode = false) {
    // Check for equality
    if (text1 === text2) {
        if (text1) {
 ...
 * @return {Array} Array of diff tuples.
 */
function bisectSplit(text1, text2, x, y) {
...
function halfMatchI(longtext, shorttext, i) {
    ...
    while ((j = shorttext.indexOf(seed, j + 1)) !== -1) {
        ...
        let suffixLength = commonSuffix(longtext.substring(0, i), shorttext.substring(0, j));
 ...
 */
function cleanupMerge(diffs, fixUnicode) {
