...
rome_1.test('compareTwoStrings', (t) => {
    const testData = [
    ...
    testData.forEach((td) => {
        t.is(orderBySimilarity_1.compareTwoStrings(td.first, td.second), td.expected);
    });
});
rome_1.test('orderBySimilarity', (t) => {
    t.looksLike(orderBySimilarity_1.orderBySimilarity('french', ['quebec', '123', 'france', 'french']), [
        { target: 'french', rating: 1 },
        { target: 'france', rating: 0.4 },
        { target: 'quebec', rating: 0 },
        { target: '123', rating: 0 },
    ]);
    t.looksLike(orderBySimilarity_1.orderBySimilarity('iphone', ['ipod', 'iphone 5s', 'iphone x']), [
    ...
    t.looksLike(orderBySimilarity_1.orderBySimilarity('iphone', ['ipod', 'iphone 5s', 'iphone x', 'IPHONE'], {
        minRating: 0.5,
    }), [
        { target: 'iphone x', rating: 0.9090909090909091 },
        { target: 'iphone 5s', rating: 0.8333333333333334 },
    ]);
    t.looksLike(orderBySimilarity_1.orderBySimilarity('iphone', ['IPHONE', 'iphone 5s', 'iphone x'], {
        minRating: 0.5,
        ignoreCase: true,
    }), [
