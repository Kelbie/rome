...
function readOctalCode(input, index, nextChar) {
    let char = nextChar;
    let octal = '';
    let nextIndex = ob1_1.ob1Add(index, 1);
    while (parser_core_1.isDigit(char)) {
        octal += char;
        ...
        nextIndex = ob1_1.ob1Add(nextIndex, 1);
        char = input[ob1_1.ob1Get0(nextIndex)];
    }
    if (octal === '') {
...
exports.createRegExpParser = parser_core_1.createParser((ParserCore) => class RegExpParser extends ParserCore {
    constructor(opts) {
            ...
            switch (nextChar) {
                ...
                case 'k': {
                    if (this.unicode) {
                        ...
                        let namedBackReferenceIndex = ob1_1.ob1Get0(index) + 2;
                        ...
                        if (namedBackReferenceChar === '<') {
                            ...
                            return this.finishComplexToken('NamedBackReferenceCharacter', {
                                ...
                                escaped: true,
                            }, ob1_1.ob1Coerce0(namedBackReferenceIndex));
                        }
                    }
                    return this.finishComplexToken('Character', {
                        value: 'k',
                ...
                case 'x': {
                    const possibleHex = input.slice(ob1_1.ob1Get0(index) + 1, ob1_1.ob1Get0(index) + 3);
                    // \xhh
                    if (possibleHex.length === 2 && isHex(possibleHex)) {
                        end = ob1_1.ob1Add(end, 2);
                        return this.finishComplexToken('Character', {
                ...
                case 'u': {
                    ...
                    const possibleHex = input.slice(ob1_1.ob1Get0(index) + 2, ob1_1.ob1Get0(index) + 6);
                    // \uhhhh
                    if (possibleHex.length === 4 && isHex(possibleHex)) {
                        end = ob1_1.ob1Add(end, 4);
                        return this.finishComplexToken('Character', {
                            value: String.fromCharCode(parseInt(possibleHex, 16)),
                            escaped: true,
                        }, end);
                ...
                default: {
                    ...
                    if (referenceValue !== undefined) {
                        ...
                        if (referenceValue >= 1 && referenceValue <= 99) {
                            ...
                            referenceEnd = ob1_1.ob1Add(referenceEnd, -1);
                            if (isOct(backReference)) {
                                return this.finishComplexToken('Character', {
    ...
    getGroupModifiers() {
            ...
            switch (token.value) {
                ...
                case '<': {
                    ...
                    if (nextToken.type === 'Character') {
                        ...
                        if (parser_core_1.isESIdentifierStart(nextToken.value)) {
                            let name = '';
                            // 1 is for the <
                            let skipCount = 1;
                            let targetToken = nextToken;
                            while (targetToken.type === 'Character' &&
                                parser_core_1.isESIdentifierChar(targetToken.value)) {
        ...
        this.addDiagnostic({
            description: diagnostics_1.descriptions.REGEX_PARSER.INVALID_CAPTURE_GROUP_MODIFIER,
            token,
        });
        return undefined;
    }
    ...
    parseGroupCapture() {
        ...
        if (!this.eatOperator(')')) {
            this.addDiagnostic({
                description: diagnostics_1.descriptions.REGEX_PARSER.UNCLOSED_GROUP,
                start,
            });
        }
    ...
    parseCharSet() {
        ...
        if (!this.eatOperator(']')) {
            this.addDiagnostic({
                description: diagnostics_1.descriptions.REGEX_PARSER.UNCLOSED_CHAR_SET,
                start,
            });
        }
        return {
    ...
    parseCharacter() {
        ...
        if (token.type === 'NamedBackReferenceCharacter') {
            const start = this.input.slice(0, ob1_1.ob1Get0(token.start));
            this.nextToken();
            if (token.value[token.value.length - 1] !== '>') {
                this.addDiagnostic({
                    description: diagnostics_1.descriptions.REGEX_PARSER.UNCLOSED_NAMED_CAPTURE,
                    loc: this.finishLocFromToken(token),
                });
            }
            if (!start.includes(token.value)) {
                this.addDiagnostic({
                    description: diagnostics_1.descriptions.REGEX_PARSER.INVALID_NAMED_CAPTURE,
                    loc: this.finishLocFromToken(token),
                });
            }
        ...
        if (start.type === 'RegExpCharacter' &&
            ...
            if (lookaheadToken.type === 'Character') {
                ...
                if (start.type === 'RegExpCharacter' &&
                    ...
                    this.addDiagnostic({
                        description: diagnostics_1.descriptions.REGEX_PARSER.REVERSED_CHAR_SET_RANGE,
                        loc,
                    });
                    const _end = end;
                    end = start;
    ...
    parseDigits() {
        ...
        while (token.type === 'Character' && parser_core_1.isDigit(token.value)) {
            digits += token.value;
                ...
                if (nextToken.type === 'Character' && nextToken.value === ',') {
                    ...
                    if (endToken.type === 'Operator' && endToken.value === '}') {
                        ...
                        if (max !== undefined && min > max) {
                            this.addDiagnostic({
                                description: diagnostics_1.descriptions.REGEX_PARSER.REVERSED_QUANTIFIER_RANGE,
                                start,
                                end,
                            });
                            return {
    ...
    parseOperator(token) {
        switch (token.value) {
            ...
            case ')': {
                this.nextToken();
                this.addDiagnostic({
                    description: diagnostics_1.descriptions.REGEX_PARSER.UNOPENED_GROUP,
                    token,
                });
                return;
            }
            case '{': {
                ...
                if (unmatchedQuantifier === undefined) {
                    ...
                    const end = this.getPosition();
                    this.addDiagnostic({
                        description: diagnostics_1.descriptions.REGEX_PARSER.NO_TARGET_QUANTIFIER,
                        start,
                        end,
                    });
                    return;
            ...
            case '+': {
                this.nextToken();
                this.addDiagnostic({
                    description: diagnostics_1.descriptions.REGEX_PARSER.INVALID_QUANTIFIER_TARGET,
                    token,
                });
                return;
            }
    ...
    parseBodyItemPrefix() {
        ...
        this.addDiagnostic({
            description: diagnostics_1.descriptions.REGEX_PARSER.UNKNOWN_REGEX_PART,
            token,
        });
        return undefined;
    }
    parseExpression(whileCallback) {
        ...
        if (expression === undefined) {
            throw new Error('Impossible. We should always have at least one alternation that will set this.');
        }
        return expression;
    }
    parse() {
