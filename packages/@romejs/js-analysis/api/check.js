...
async function check(opts) {
    const hub = await buildGraph_1.default({
        ast: opts.ast,
        connected: true,
...
function isError(t) {
    return t !== undefined && t instanceof E_1.default;
}
function resolveGraph(hub) {
    ...
    for (const node of graph.nodes) {
        ...
        if (lower instanceof OpenT_1.default) {
            continue;
        }
        // see if this reduces to a type error
        const reduced = utils.reduce(lower);
        if (reduced instanceof E_1.default) {
            ...
            if (upperTarget !== undefined) {
                const marker = upperTarget && !(upperTarget instanceof reduced.constructor)
        ...
        for (const line of node.lines) {
            ...
            if (compatibility.type === 'incompatible') {
                ...
                context.addNodeDiagnostic(compatibility.lower.originNode, diagnostics_1.descriptions.TYPE_CHECK.INCOMPATIBILITY(utils.humanize(upper), upper.originLoc), {
