...
const provider = js_compiler_1.createHook({
    ...
    exit(path, state) {
        for (const name in state.usedBindings) {
            ...
            if (used === false && binding !== undefined) {
                path.context.addNodeDiagnostic(binding.node, diagnostics_1.descriptions.LINT.UNUSED_VARIABLES(binding.kind, name));
            }
        }
        return path.node;
    },
    ...
    enter(path) {
        ...
        if (scope.node === node) {
            ...
            for (const [name, binding] of scope.getOwnBindings()) {
                if (binding instanceof bindings_1.ArgumentsBinding) {
                    continue;
                }
            ...
            if (node.type === 'FunctionDeclaration' ||
                ...
                for (const { name } of js_ast_utils_1.getBindingIdentifiers(node.head.params.slice(0, -1))) {
                ...
                if (block.type === 'BlockStatement' &&
                    ...
                    for (const { name } of js_ast_utils_1.getBindingIdentifiers(node.head.params)) {
                        usedBindings[name] = true;
                    }
                }
            }
