...
function getName(node) {
    ...
    if (property.type === 'ComputedMemberProperty') {
        ...
        if (js_ast_utils_1.isIdentifierish(property)) {
        ...
        return path.reduce({
            ...
            enter(path) {
                ...
                if ((node.type === 'MemberExpression' ||
                    ...
                    js_ast_utils_1.isIdentifierish(node.object)) {
                    ...
                    if (wildcardInfo !== undefined && wildcardInfo.references.has(node)) {
                        ...
                        if (node.type === 'JSXMemberExpression') {
                            return js_ast_1.jsxIdentifier.quick(newName);
                        }
                        else {
                            return js_ast_1.referenceIdentifier.quick(newName);
                        }
                    }
                }
                ...
                if (node.type === 'ImportDeclaration' &&
                    ...
                    for (const [imported, local] of wildcardInfo.mappings) {
                        namedSpecifiers.push(js_ast_1.importSpecifier.create({
                            imported: js_ast_1.identifier.quick(imported),
                            local: js_ast_1.importSpecifierLocal.quick(js_ast_1.bindingIdentifier.quick(local)),
                        }));
                    }
                    return js_ast_1.importDeclaration.create({
                        ...node,
